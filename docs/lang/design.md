...
- 🧑 ✅ A file path in a payload (the `input` of a `reading file`/cycle error) is **relative to `Dir.pwd`** (the directory the command was invoked from), so it reads as the route from where you are to the offending file — directly usable, and free of any machine-specific absolute prefix. A Ruby `Errno` message is reduced to its strerror (`"Is a directory"`), dropping the `@ syscall - path` tail (the path is already in `input`).

...
- 🧑 ✅ A file path in a payload (the `input` of a `reading file`/cycle error) is **relative to `Dir.pwd`** — no machine-specific absolute prefix, and stable when a whole project moves together. A Ruby `Errno` message is reduced to its strerror (`"Is a directory"`), dropping the `@ syscall - path` tail (the path is already in `input`).

...
- 🧑 ✅ `@sanitize` recurses through arrays (via `@map`) and objects (via `@mapValues`), so an unserializable value nested at **any** depth is rewritten — no residual `serialization_error` from a deep function/non-finite. Key-preserving object reconstruction is not expressible from `@keys` alone (Fusion has no computed-key object literal), so three object primitives were added as built-ins: **`@get`** `[obj, key]`, **`@set`** `[obj, key, value]` (returns a new object), and **`@toObject`** `[[key, value], …]`. `@mapValues` (stdlib) is built on `@keys` + `@map` + `@toObject`.

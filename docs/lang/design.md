...
- 🧑 ✅ `origin` (one of `builtin`, `stdlib`, `code`, `input`, `output`, `interpreter`) is where the operation is *defined*. The optional `file` is its **call site** — independent of `origin`, so a builtin/stdlib operation called from your code carries *your* `file` (`"<inline>"` for inline `-e`/REPL); absent when the operation is reached from stdlib internals or has no code call site.
- 🧑 ✅ `operation` is the failing operation's own **`@`-reference** for a builtin or stdlib function (`@add`, `@math/square`, `@OP.add`) — exactly the expression that retrieves that function, so `[failed_fn, @ref] | @equals` holds. Built-in *syntax* names its form instead (`|`, `.key`, `[]`, `parsing code`, …).
...
- 🧑 ✅ A failure to *reach* a file (`reading file`, cycle) takes the **referring** call site as its `file`, with `input` echoing the user's own `@`-reference / `@load` argument unmodified — never the resolved path. A Ruby `Errno` message is reduced to its strerror (`"Is a directory"`), dropping the `@ syscall - path` tail.

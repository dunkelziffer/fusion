...
The `file` is written **relative to the directory you ran the command from**, so it reads as the route to the code nearest the failure. A failure to *reach* a referenced file (`reading file`, or a cyclic reference) is likewise reported against the **referring** code, with `input` echoing your own `@`-reference (or `@load` argument) exactly as written — never the resolved path.

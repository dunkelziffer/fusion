  ...
  describe "above all user code (\"<fusion>\")" do
    ...
    it "a stdlib function applied directly to the input" do
      expect_pipe
        .in("✅", "5")
        .code("@map")
        .out("❌", '{"kind":"argument_error","origin":"stdlib","file":"<fusion>","operation":"@map","status":0,"input":5,"expected":["{\"f\": _ ? @Function, \"c\": _ ? @Collection}"]}')
    end
  ...
  describe "in a user file" do
    ...
    it "a stdlib function piped directly" do
      expect_pipe
        ...
        .out("❌", '{"kind":"argument_error","origin":"stdlib","file":"spec/fixtures/callsite/stdlib.fsn","operation":"@map","status":0,"input":5,"expected":["{\"f\": _ ? @Function, \"c\": _ ? @Collection}"]}')
    end

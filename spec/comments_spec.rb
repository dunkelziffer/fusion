  ...
  describe "rejected forms" do
    it "rejects a trailing inline comment" do
      expect_pipe
        ...
        .out("❌", a_string_including(
          '"kind":"syntax_error"', '"origin":"code"', '"file":"<inline>"', '"operation":"parsing"', '"status":0', '"input":"(_ => 1) # nope"',
          /"message":"(?:[^"\\]|\\.)*"/
        ))
    end
    ...
    it "rejects a raw newline inside a string" do
      expect_pipe
        .code("(_ => \"line1\nline2\")")
        .out("❌", a_string_including(
          '"kind":"syntax_error"', '"origin":"code"', '"file":"<inline>"', '"operation":"parsing"', '"status":0', '"input":"(_ => \"line1\nline2\")"',
          /"message":"(?:[^"\\]|\\.)*"/
        ))
    end
  end

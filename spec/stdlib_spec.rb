  ...
  describe "@math/square" do
    ...
    it "errors on a non-integer (string)" do
      expect_pipe
        .in("✅", '"hi"')
        .code("(x => x | @math/square)")
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"square.fsn","operation":"square","status":0,"input":"hi","expected":["_ ? @Integer"]}')
    end
    ...
    it "errors on a float (square is integer-only)" do
      expect_pipe
        .in("✅", "2.5")
        .code("(x => x | @math/square)")
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"square.fsn","operation":"square","status":0,"input":2.5,"expected":["_ ? @Integer"]}')
    end
  end
  ...
  describe "@range" do
    ...
    it "errors on a non-integer" do
      expect_pipe
        .in("✅", '"hi"')
        .code("(x => x | @range)")
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"range.fsn","operation":"range","status":0,"input":"hi","expected":["_ ? (m ? @Integer => [-1, m] | @lessThan, _ => false)"]}')
    end
    ...
    it "errors on a negative integer (rather than recursing forever)" do
      expect_pipe
        .in("✅", "-1")
        .code("(x => x | @range)")
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"range.fsn","operation":"range","status":0,"input":-1,"expected":["_ ? (m ? @Integer => [-1, m] | @lessThan, _ => false)"]}')
    end
  end
  ...
  describe "@map" do
    ...
    it "errors on a non-{f,xs} value" do
      expect_pipe
        .in("✅", "5")
        .code("(x => x | @map)")
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"map.fsn","operation":"map","status":0,"input":5,"expected":["{\"f\": _, \"xs\": _ ? @Array}"]}')
    end
    ...
    it "errors when a required key is missing" do
      expect_pipe
        .in("✅", "[1,2]")
        .code('(xs => {"xs": xs} | @map)')
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"map.fsn","operation":"map","status":0,"input":{"xs":[1,2]},"expected":["{\"f\": _, \"xs\": _ ? @Array}"]}')
    end
    ...
    it "errors when xs is present but not an array" do
      expect_pipe
        ...
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"map.fsn","operation":"map","status":0,"input":{"f":"<function>","xs":"nope"},"expected":["{\"f\": _, \"xs\": _ ? @Array}"]}')
    end
  end
  ...
  describe "unserializable inputs are sanitized in the echoed payload" do
    it "@math/square of a function reports an argument_error, not a serialization_error" do
      expect_pipe
        ...
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"square.fsn","operation":"square","status":0,"input":"<function>","expected":["_ ? @Integer"]}')
    end
    ...
    it "@math/square of a non-finite number echoes the placeholder" do
      expect_pipe
        ...
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"square.fsn","operation":"square","status":0,"input":"<Infinity>","expected":["_ ? @Integer"]}')
    end
    ...
    it "@range of a non-finite number echoes the placeholder" do
      expect_pipe
        .in("✅", "null")
        .code("(_ => 1e400 | @range)")
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"range.fsn","operation":"range","status":0,"input":"<Infinity>","expected":["_ ? (m ? @Integer => [-1, m] | @lessThan, _ => false)"]}')
    end
    ...
    it "@map of a {f} missing xs echoes the function placeholder" do
      expect_pipe
        .in("✅", "null")
        .code('(_ => {"f": @negate} | @map)')
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"map.fsn","operation":"map","status":0,"input":{"f":"<function>"},"expected":["{\"f\": _, \"xs\": _ ? @Array}"]}')
    end
    ...
    it "@map of a bare function echoes the function placeholder" do
      expect_pipe
        .in("✅", "null")
        .code("(_ => (y => y) | @map)")
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"map.fsn","operation":"map","status":0,"input":"<function>","expected":["{\"f\": _, \"xs\": _ ? @Array}"]}')
    end
    ...
    it "sanitizes deeply nested functions and non-finite numbers in the echoed input" do
      expect_pipe
        ...
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"map.fsn","operation":"map","status":0,"input":{"a":[1,"<function>",{"deep":"<function>"}],"b":["<Infinity>"]},"expected":["{\"f\": _, \"xs\": _ ? @Array}"]}')
    end
  end
  ...
  describe "@mapValues" do
    ...
    it "errors when object is not an object" do
      expect_pipe
        .in("✅", "5")
        .code('(o => {"f": (n => n), "object": o} | @mapValues)')
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"mapValues.fsn","operation":"mapValues","status":0,"input":{"f":"<function>","object":5},"expected":["{\"f\": _, \"object\": _ ? @Object}"]}')
    end
    ...
    it "errors on a non-{f,object} value" do
      expect_pipe
        .in("✅", "5")
        .code("(x => x | @mapValues)")
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"mapValues.fsn","operation":"mapValues","status":0,"input":5,"expected":["{\"f\": _, \"object\": _ ? @Object}"]}')
    end
  end
  ...
  describe "@all" do
    ...
    it "errors on a non-{f,xs} value" do
      expect_pipe
        .in("✅", "5")
        .code("(x => x | @all)")
        .out("❌", '{"kind":"argument_error","location":"stdlib","file":"all.fsn","operation":"all","status":0,"input":5,"expected":["{\"f\": _, \"xs\": _ ? @Array}"]}')
    end
  end
end

  ...
  describe "duplicate binders" do
    ...
    it "rejects a repeated binder across object members" do
      expect_pipe
        .in("✅", '{"x":1,"y":2}')
        .code('({"x": v, "y": v} => v)')
        .out("❌", '{"kind":"binding_error","location":"code","file":"<inline>","operation":"binding identifier v","status":0,"input":"v","message":"identifier already bound"}')
    end
  ...
  describe "guards (?)" do
    ...
    it "does not let the predicate expression see the pattern's sibling binders" do
      ...
      expect_pipe
        .in("✅", "[1,2]")
        .code("([a, b ? a] => \"matched\")")
        .out("❌", '{"kind":"binding_error","location":"code","file":"<inline>","operation":"reading identifier a","status":0,"input":"a","message":"unbound identifier"}')
    end

  ...
  describe "member access" do
    ...
    it "errors on a missing key" do
      expect_pipe
        .in("✅", '{"name":"bob"}')
        .code("(o => o.nope)")
        .out("❌", '{"kind":"access_error","location":"code","operation":".nope","status":0,"input":[{"name":"bob"},"nope"],"message":"missing key"}')
    end
  end
  ...
  describe "index access" do
    ...
    it "errors when the index is out of range" do
      expect_pipe
        .in("✅", "[10,20,30]")
        .code("(a => a[9])")
        .out("❌", '{"kind":"access_error","location":"code","operation":"[9]","status":0,"input":[[10,20,30],9],"message":"index out of range"}')
    end
  end
end
